...
#from getServices import analyzer
...
def nlpMask(content,prefix):
    print("Applying NLP!!!")
    #analyzer = AnalyzerEngine()
    global analyzer

    try:
        results = analyzer.analyze(correlation_id=0,
                           text= content,
                           entities=[prefix],
                           language='en',
                           )
...
def numbersOnlyData(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]

        if suffix == 'NLP':
            results = nlpMask(data,prefix)
            repl = {}
            for item in results:
                pidata = data[item.start:item.end]
                length = len(pidata)
            ...
            data = pattern.sub(lambda m: repl[re.escape(m.group(0))], data)
...
def charsOnlyData(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def numbersAndChars(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def maskWithAsterix(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def maskWithHash(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def reduceMaskWithAsterix(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def reduceMaskWithHash(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def partialMaskWithAsterix(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def partialMaskWithHash(encoding,content,prefix,suffix,unmask,sessionid):
    try:
        data = content.decode(encoding)
        mask_pairs = [['cookie',sessionid]]
...
def emailData(encoding,content,prefix,suffix,unmask,sessionid):
